...
** TODO Boundary conditions [0/0]
  - [ ] Two NE_0 model - Just for competition model
...
** TODO Tools, solving, and fitting [0/6]
  ...
  - [ ] Solving
    - [ ] scipy.odeint -> optimized by vectorizing and using numpy
      - [ ] atol=1.49012e-8, rtol=1.49012e-8, mxhnil=0
    - [ ] libroadrunner
    - [ ] must write models in SBML (libSBML) and, unlike odeint,
    ...
    - [ ] QFA data for the Stripes plate contained cell observations
      for cultures that were known to be empty. I set b to zero for
      these cultures and removed them from the objective function.
    - [ ] Cultures at the edge of a plate have an advantage because
      they have access to a greater area of nutrients which could
      diffuse from outside of the array.
      - (Data from these cultures is noisey due to reflections from
        plate edges. This is only partially corrected for by Colonyzer
        (ref) and as a result data from edge cultures is useually
        discarded. Addniall et al grow the a repeats of the same
        culture in edge locations, rather than leaving them empty,
  ...
  - [ ] Fitting the Competition Model
    - [ ] SciPy's scipy.optimize.minimize (optimization and rootfinding package)
    - [ ] Given a model and data, I made maximum likelihood estimates
      of parameters using a normal model of measurement error.
    - [ ] QFA R package for logistic model - Contains heuristic checks
      for slow growing cultures and is currently used in QFA.
    - [ ] Should I talk about the Development Process? (i.e. use of
