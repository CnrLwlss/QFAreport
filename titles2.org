...
* TODO Discussion [0/4]
** TODO Discuss results and place in context of other work [0/4]
 - [ ] P15
   - [ ] Fitness rankings broadly agree
     - [ ] I could also do with correlation plots between model r as I
       have for the stripes and filled fits to make a comparison
     - [ ] Agree with validation spot data (add refs) - detail in the
       results.
     - [ ] (Wait untill COV to discuss differences in r and MDR of
       logistic QFA.)
   - [ ] COV
     - [ ] COV higher in Competition model for fastest growers
     - [ ] COV lower in competiton model for other cultures and slowest
       growers (dominated by noise)
     - [ ] Probably due to sharing of plate level parameters, but not
       necessarily modelling of competition -> Must use data from
       noise dominated.
     - [ ] QFA R reqsuires hueristic checks for slowest growers due to
       confounding between r and K.
     - [ ] Still some issues - Encroachment not picked up by Colonyzer
       of QFA R - est1 fitted with a much higher r and K
     - [ ] Order corrected when converting to MDR creating confusion
       about which fitness estimate to use.
     - [ ] Could be corrected either experimentally, in Colonyzer, or in
       QFA R using a heuristic check for late growers.
     - [ ] Competition model rank of slow growers probably more trustworthy
     - [ ] QFA R uses culture level C_0 (may or may not be more
       accurate)
   - [ ] HETEROGENEITY: Herrmann and Lawless:
     - [ ] Due to cell heterogeneity we believe that C_0 varys between
       cultures.
     - [ ] Makes direct measurement of C_0 unreliable
     - [ ] Allowing a culture level C_0 more accurate but preferable
       if were similar
     - [ ] Strategies:
       - [ ] Experimental:
	 - [ ] Inoculate cells taken from the exponential growth phase
           to reduce heterogeneity
	 - [ ] Use higher starting density to try to average out
           heterogeneity
       - [ ] Analysis:
	 - [ ] Then: Fit C_0 collectively -> 1 parameter.
	 - [ ] Or: Only small amount of nutrients used at start -> Can
	 ...
	 - [ ] Or: Fit separately without direct measurement -> 384
           parameters
   - [ ] Conclusion: Failure to improve reliability over Addinall et
 ...
 - [ ] Stripes and Filled
   - [ ] Starts with higher inoculum so no noise-dominated cultures.
   - [ ] No heuristic checks needed for logistic when no noise dominated
   - [ ] Performance of competition model improved and similar to
     logistic model. Need correlation plot for P15 to compare.
     - [ ] Despite not finding global minima
   - [ ] However correlation between models is now worse
   - [ ] Validation issues with both models.
     - [ ] Logistic does not correct at all for competition
     - [ ] Competitoin model overcorrects
   - [ ] No repeats on this plate so cannot study COV
   - [ ] How can we determine which estimates to believe?
     - [ ] Do we lack validation data for this plate?
     - [ ] Could do with new experiments
       - [ ] Softer validation
       - [ ] Repeat of P15 with higher C_0.
       - [ ] Ideally leave edge cultures empty
 - [ ] Issues with fitting
   - [ ] Not finding global minina
   - [ ] Could try genetic algorithm or Bayesian approach of
     Heyderi et al.
     - [ ] Bayesian likely to be slower
     - [ ] I have found that fixing plate level parameters allows
       global minima to be found so could possibly use hierarchical ga
       with candidates of plate level parameters. Could be faster.
   - [ ] However, fitness rankings are similar for best fits and also
     make overestimates in validation so likely to be a more
     fundamental issue with the model.
 - [ ] Conclusion:
   - [ ] Competitoin model appears to perform better and has
     different order to logistic (poor correlation)
   - [ ] Probablyrequires futher validation using cultures with
     inoculated higher density, repeats, and known biology. Perhaps a
     repeat of P15.
   - [ ] In any case the competitoin model still needs improving


** TODO Future Work [0/3]
 ...
 - [ ] Validate the independent limit
   ...
   - [ ] Mass action kinetics (I have a ref for this if needed)
     - [ ] more valid for liquid cultures
   - [ ] Liquid cultures (paper)
   - [ ] Isolated spots
     - [ ] Different dimensions - Issues with diffusion limited and
       surface (Fractal kinetics may be required (ref))
     - [ ] Cultures not uniform (ring around outside)
     - [ ] cultures grow differently between mediums but ok. Same
       model different parameters.
   - [ ] Mass action may be more valid in liquid so could use the mass
     action logistic model and \(b\) as fitness estimate.
   ...
   - [ ] Differences between nutrient density.
     ...
     - [ ] I believe diffusion is limited in supply from agar
       - [ ] that nutrients are not well approximated as being evenly
     ...
     - [ ] Design agar to limit a certain nutrient that is not
       metabolised.
   - [ ] Our fits contain fairly high values of kn (fast diffusion)
     may be inaccurate
   - [ ] Growth may stop before nutreints diffuse form neighbour.
     - [ ] Can experimentally determine this also by growing new
   ...
   - [ ] Diffusion equation model to study point where growth is
     limited by supply/diffusion of nutrients through agar.
     - [ ] A diffusion equation model could capture the local
       distribution of nutrients around a culture when the stationary
       phase is reached
     - [ ] I have a good section on Reo and Korolev that should go in
       either the introduction or here.
   - [ ] May require finer grid.
   - [ ] Signal could instead/also be modelled
     - [ ] Ethanol or quorum sensing (ref)
   - [ ] Arrest. Sick cells may be arresting
   - [ ] Combination of effects hard to fit without extra observations
...
** TODO Improvements to current code / workflow [0/4]
  - [ ] loop through b_guess in Imag neigh guessing
  - [ ] Alternatively could use logistic model as init guess
  - [ ] Edge cultures [0/2]
    - [ ] Discussion: An alternative model could have added nutrient
      ...
      noise.
    - [ ] Discussion: When fitting the competition model, it would be
      better if these cultures were left empty to remove noise in cell
      density observations.
  - [ ] To make development easier use smaller isolated zones inside a
