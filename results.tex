...
strains in the Stripes plate are inoculated in the same postitions on
...
Discuss issues with validation plate.
...
these timecourses from the objective function when fitting. The
experiment used the same inoculum density and formula of agar between
plates so I do not expect plate level parameters vary between
plates. However, there is significant disagreement in all parameters.
...
(Table~\ref{tab:Stripes_and_f}). Despite this, the estimates from the
top five Filled plate fits all exhibited the same behaviour of
overestimating growth in used to simulate the Stripes plate.
