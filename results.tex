...
\begin{Figure}
  \centering
  \includegraphics[width=\linewidth]{final/P15_r_correlations_median_rho_and_rs}
  \captionof{figure}{\textbf{Correlation between logistic and
      competition model r estimates for P15}. Correlations are plotted
    between r estimates for cultures (black) and between the medians
    ...
    R package. The Pearson correlation coefficient, \(\rho\), and
    Spearman rank correlation coefficient, \(r_{s}\), are shown for
    each distribution. The line y = x is also plotted.}
...
% \columnbreak
% \begin{center}
%   \captionof{table}{\textbf{Pearson correlation coefficient and
%       Spearman rank correlation coefficient for logistic model and
%       competition model estimates of \(\bm{r}\) and \(\bm{MDR}\) for
%       P15.} Correlation coefficients compare logistic and competition
%     model estimates of parameters \(r\) and \(MDR\). Correlations are
%     shown between estimates for each culture and between median and
%     mean estimates for each deletion.}
% \begin{tabular}{ |c|c|c|c| }
% \cline{3-4}
% \multicolumn{2}{c|}{} & \(r\) & \(MDR\)  \\ \hline
%   \multirow{3}{*}{Pearson~\(\rho\)} & Cultures & 0.712 & 0.752 \\
% & Median & 0.708 & 0.791 \\
% & Mean   & 0.763 & 0.885 \\ \hline
%   \multirow{3}{*}{Spearman~\(r_{s}\)} & Cultures & 0.731 & 0.754\\
% & Median & 0.497 & 0.635 \\
% & Mean   & 0.594 & 0.732 \\ \hline
% \end{tabular}
% \end{center}
...
\begin{center}
  \captionof{table}{\textbf{Correlation coefficients between logistic
    ...
    correlation coefficient \(\bm{\rho}\) and Spearman rank
    correlation coefficient \(\bm{r_{s}}\) are shown. Correlation
    coefficients compare between logistic and competition model
    estimates of both \(r\) and \(MDR\). Correlations are shown
    between estimates for each culture and between median and mean
    estimates for each deletion.}
\begin{tabular}{ |c|c|c|c|c| } \cline{2-5}
\multicolumn{1}{c|}{} & \multicolumn{2}{c|}{\(r\)} & \multicolumn{2}{c|}{\(MDR\)}\\ \hline %\cline{2-5}
\multicolumn{1}{|c|}{Estimate} & \(\rho\) & \(r_{s}\) & \(\rho\) & \(r_{s}\) \\ \hline
