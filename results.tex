...
better model should return more similar fitness estimates for repeats
of the same strain, regardless of where they grown on the plate.

Deletions in Figure~\ref{fig:COV} are arranged from left to right
according to competition model\(b\) ranking, with the fittest deletion
at the left. The competition model has smaller COV in 36 out of 50
deletions. The logistic model has a smaller COV for the 11 fittest
deletions (according to \(b\) ranking). Comparing with
Figure~\ref{fig:P15_correlations} these deletions (medians) appear in
the upper right of both groups and not in the gap inbetween. \(b\) COV
becomes the lower than \(r\) at smaller values of \(b\), where repeats
are likely divided between the groups. This is indicated by the
positions medians inbetween groups. The slowest growers tend to have
greater COV for both models which is likely due to the greater effect
of noise in these cultures.
