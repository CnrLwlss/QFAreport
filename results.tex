...
I compared the competition and logistic model by fitting both to P15
...
is to be used as a fitness estimate to compare with the logistic model
(see Section~\ref{sec:parameter_conversion}).
...
When comparing objective function values for internal cultures, used
to determine best fits, the logistic model is slightly better (see
Table~\ref{tab:P15_obj_fun}).
% The total objective function value for internal cultures is also
% slightly better for the logistic model (0.155) compared to the
% competition model (0.194).
The competition model performs well considering that it used far fewer
parameters (387 vs 1152).
% Methods or here or to top of paragraph?
N.B. To make objective function comparisons fair, I converted logistic
model parameters to competition model parameters by
(\ref{eq:conversion}) and resimulated both models using CANS.
...
\subsection{Comparison of Variation in Fitness Estimates}
