...
To compare estimates between models, I converted competition model
\(b\) to logistic model \(r\) using (\ref{eq:conversion}).
...
\(r \approx 4\). This creates two distinct correlated groups: the
first with lower \(r\) and gradient close to one, the second with
higher \(r\) and a steeper gradient. Cultures from the middle of the
...
outliers with very high logistic \(r\) are repeats of
\textit{rad50\(\Delta\)} and \textit{est1\(\Delta\)}. They have
escaped the heuristic checks and exhibit a confounding effect between
\(r\) and \(K\).
...
are split between the groups. Correlation, measured by Spearman's rank
correlation coefficient, \(\rho_{S}\), is lower between medians
(0.497) than between cultures (0.731).
...
\begin{Figure}
  ...
  \captionof{figure}{\textbf{Correlation between logistic and
    ...
    coefficient, \(\rho_{s}\), is shown for each distribution. The
