...
I compared the precision of competition and logistic model fitness
estimates using the coefficient of variation (COV) in \(r\) for
repeats of each deletion (Figure~\ref{fig:COV}). N.B. COV of
competition \(b\), \(r\), and \(MDR\) are equivalent. I chose to
compare logistic \(r\), rather than \(MDR\), because both \(r\) and
\(b\) are fundamental parameters: growth constants; \(MDR\) is
dependent on initial conditions (see
Equation~\ref{eq:MDR_MDP}a). Regardless, COVs for logistic \(r\) and
...
% of \(r\) between repeats of each deletion on P15.
I chose to compare \(b\) and \(r\), rather than \(b\) and \(MDR\),
because both parameters represent growth constants. The precision of fitness estimates
is of interest both as a test of the model and because it affects the
power to infer genetic interactions. If we assume that biological
variation in fitness between repeats of the same strain is small, then
the better model should estimate the fitness of strains more
precisely, regardless of where repeats are grown on the plate.
