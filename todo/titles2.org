* TODO Abstract [0/17]
 - [ ] I find that a mass action kinetic model of nutreint dependent
   growth and diffusion... fits cell density timecourse from a
   384-format QFA plate inoculated with dilute cultures with similar
   accuracy to (a model currently used in QFA which assumes cultures
   grow independently) a model currently used in QFA, which assume
   that culture grow independently,( of the logistic growth model
   )using far fewer parameters (387 vs 1152).
 - [ ] The logistic growth model has
   previously been used to fit QFA data and assumes that cultures grow
   independetly.
 - [ ] I use a gradient method to collectively fit a mass action
   kinetic model of nutreint dependent growth and diffusion to cell
   density timecourses from a 384-format QFA plate inoculated with
   dilute cultures. I find that the model fits timecourses with
   similar accuracy to independent fits of the logistic growth model,
   which assume cultures grow independently, which assumes using far
   fewer parameters (387 vs 1152).

 - [ ] Fits of this model are of a similar quality to fits of a
   model currently used in QFA that assumes cultures grow
   independently and use far fewer parameters (387 vs 1152).
 - [ ] Fits of this model are similar in quality to fits of a
   model currently used in QFA that assumes cultures grow
   independently and use far fewer parameters (387 vs 1152).

  - [ ] Fits of this model are similar in quality to fits of the
   logistic growth model, which assumes cultures grow indenpendently,
   currently used in QFA.  and use far fewer parameters (387 vs 1152).


 - [ ] Fits of this model are similar in quality to fits using a
   current QFA model that assume cultures grow indenpendently and use
   far fewer parameters (387 vs 1152).

 - [ ] Using far fewer parameters (387 vs 1152), this model fits
   timecourses with a similar accuracy to a previous QFA model which assumed cultures grow independently.
 assuming that
   cultures grow indenpendently which was previously used to fit QFA
   data, that assumes cultures grow indenpendently, and use far fewer
   parameters (387 vs 1152).

 - [ ] I use a gradient method to collectively fit a mass action
   kinetic model of nutreint dependent growth and diffusion to cell
   density timecourses from a 384-format QFA plate inoculated with
   dilute cultures.
 - [ ] Using far fewer parameters (387 vs 1152), the model fits
   timecourses with a similar accuracy to a (current QFA) model which
   assumes that cultures grow independently. Accuracy is improved for
   fast growing strains.
 - [ ] Fitness rankings of diffrent strains on the same plate agree
   with a previous QFA study and independent experiments in the
   positions of the fastest and slowest growing strains, but disagree
   for medium growing strains.
 - [ ]  Estimates are less presise for the fastest growing strains,
   but more precise for 36 out of 50 strains.
 - [ ] In cross plate callibration and validation, using plates with a
   higher inoculum density, the model overcorrects for competiton
   effects to a similar degree that current models undercorrect.
 - [ ] A different method of fitting will be required to find globally
   optimatal solutions.


 - [ ] model currently used in QFA which assume that
   cultures grow independently while using far fewer parameters (387
   vs 1152)

 - [ ] A more mechanistic model of nutrient dependentcorrecting for competition for
   nutrients between neighbouring cultures fits cell density
   timecourse from a QFA plate inoculated with dilute cultures with
   similar accuracy to the logistic growth model.
 - [ ] Inferred Timecourses fit data with a similar quality as the
   logistiac growth model despite using around 3x fewer parameters.
 - [ ] a plate with an objective function value quality inferred
   Inferred
*
...
* TODO Results [0/7]
 - [ ] Describe experiments
   - Should probably do this in an earlier section first so that I am
     not repeating myself.
 - [ ] Zone fit
   - [ ] bottom left bad fit for init guess
   - [ ] Very good fit
   - [ ] Nutrients in bottom right greater at all timepoints
   - [ ] obj function value
   - [ ] Could compare to QFA R logistic model (I have the parameters
     to convert)
 - [ ] Whole plate
   - [ ] Fits well across whole plate
   - [ ] Nutrients depleted at the end of  observations
 - [ ] Corner
   - [ ] Two N_0 fits a higher starting amount of nutrients
   - [ ] Lower values of objective function are better
   - [ ] Results in lower objective function value for internal
     cultrures next to edge cultures and lower objective function
     overall.
   - [ ] Appears to be working.
     - Due to collective fit actually greater objective function
       values for edges. These are discarded due to noise. Appears to
       be working as we hoped.
   - [ ] Notable that edge cultures objective function is actaully increased
     - [ ] Much more noise as seen in top left
     - [ ] Fitting collectively
     - [ ] Edges are discarded
 - [ ] Correlation plots for best fits or just mention and give
   Spearman's values
 - [ ] (priority) Correlation plots for comp model and logistic model for P15
 - [ ] Ranking
   - [ ] I used mean - Try median
   - [ ] Bold HIS3 and p-values
   - [ ] Where do and don't things agree?
   - [ ] Describe b, r, and MDR
   - [ ] Validation (sumarise in a table?) - I think it should be ok
     just to mention it and that it agreed.
     - Could mention issues with temperature differences and lack of validation for some
     - [ ] Disagreement with mre11 anomolous. Try to remove mre11
       repeat from data (4, 11) (with origin (0, 0))? - Comparison of
       methods so could leave in. Or just check.
   - [ ] COV
     - [ ] Describe process
     - [ ] Describe results
     - [ ] 36/50 strains more precise for the competition model.
   - [ ] Callibration and Validation
     - [ ] Describe process
     - [ ] Describe results
     - [ ] Mention inoculation error in top right
       - Bottom left far from such errors
     - [ ] blue crosses for middle column just noise and not fit
       (Could remove in plots) (Also lines)
     - [ ] logistic model no change between plates and is an
       undercorrection.
   - [ ] Correllation plots
   - [ ] Towards a genetic algorithm
     - [ ] Remove GA fit so don't have to describe
     - [ ] Describe recovery of parameters with simulated noise and
       potential use in genetic algorithm.
       - [ ] I got the parameters for the best fits and simulated with noise
       - [ ] Required several guesses
       - [ ] This is the worst of how many?
       - [ ] Harder to identify low b (noise)
       - [ ]
* TODO Discussion [0/4]
...
* TODO Describe P15 [12/12]
 - [X] Plate 15 from a standard library
 - [X] From Addinall et al 2011
 - [X] Dilute cultures of ~100 cells - not detectablen
 - [X] Deletions of genes relevant to telomere stability
 - [X] HIS3 is neutral deletion 14 repeats
 - [X] All other deletions have 6 repeats
 - [X] HIS3 is grown in edge locations and results are discarded
 - [X] cdc13-1 is a temperature conditional mutant. CDC13 is a gene
   important for telomere stability
 - [X] plate is incubated at 27 C where cdc13-1 starts to have loss of function
 - [X] How many timepoints over what time? 10 over ~4 days
 - [X] Why choose this plate - see presentation
   - [X] Lots of replicates
   - [X] Large variation in fitness of strains - hope to induce
     competition.
   - [X] There is validation data for some of the strains from
     independent spot tests. Visual inspecition of serial diluted spot
     tests with few replicates. Qualitative observations or relative
     final cell densities. Slightly different temperatures. Doesn't
     cover most of the deletions on the plate
 - [X] Validations references


* TODO Describe Stripes and Filled plates [10/10]
 - [X] Experiment designed to detect competiton effects Eva Holstein
   (personal communications April 2016)
 - [X] Grown on two plates
   - [X] Stripes: Strains are inoculated in every other column
   - [X] Filled: The same strains are inoculated in the same positions
     as in the Stripes plate. Gaps are now filled in with new
     cultures. Each new culture is a variation of the strian to the left.
 - [X] More concentrated (~10000 of cells) - detectable
 - [X] Inocculated from the same liquid cultures.
 - [X] What are the deletions?
 - [X] No repeats for most strains
 - [X] Background mutation cdc13-1 and rad57
 - [X] The left most column is all HIS3
 - [X] Temperature 27C
 - [X] How many timepoints over what time ~50 timepoints over ~4 days

* TODO p15 results
  - [-] Describe how the plate was fit [7/8]
    - [X] I ran multiple fits of the competiton model to P15.
    - [X] Each used the imaginary neighbour model as an inital guess.
    - [X] Initial parameters supplied to imag neighbour guess
    - [X] Using 5 guesses of C_0 from N_0*10-3 to N_0*10-5 in
      logspace.
    - [X] Used 14 initial parameters for \(b\) for the guesser. Guessed \(b_{i}\) for imaginary guessing model from (35, 40, ..., 100).
    - [ ] The best used \(b\) = 35 and \(C_0\) = 10-4*N_0
    - [X] Did not find local minimum but top five fits were correlated
      each other with Spearmans rank order correlation coefficient of
      (0.968 - 0.994)
    - [X] Table parameter values for best two fits.
  - [X] Describe and compare the fits [2/2]
    - [X] Comp fit to full plate appears good accross the plate
    - [X] Zone [9/9]
      - [X] Guess is typical for the plate with some cultures disagreeing
      - [X] Could also have spearmans correlation coefficient of b
        with the best guess. Could put parameter values in a table.
      - [X] Logistic model used QFA R package
      - [X] Point to table
	- [X] Objective function values similar but logistic model
          beats the competition model.
	- [X] (Dicussion?) The QFA R logistic fit uses 1152 parameters
          competition model uses 387.
      - [X] objective funstion for most cultures are similar.
      - [X] However, the competiton model fits much better to the
        central and center-right fast growing cultures.
      - [X] This zone contains more large growing cultures than is
        average for the plate. comp obj = 44.06; log obj = 68.38.
      - [X] Likely more competition in this zone.
      - [X] The logistic model is probalby fitting other areas better.
  - [X] Treatment of boundary conditions (may leave out) [6/6]
    - [X] I also fit the one initial nutrient parameter model to P15
      using the exact same method.
    - [X] plot shows the fits to the top left corner of P15 for both
      - [X] Fits appear very similar in quality.
      - [X] But look at table
    - [X] Error in cultures next to the edge was reduced with the two parameter model
    - [X] Internal cultures were fit better overall with the
    - [X] Interestingly fits of the edge cultures were actaully worse
      - [X] These are dominated by noise
      - [X] Must be fit in the competitin model
    - [X] Model is working correctly and fitting the cultures collectivly
  - [-] Correlation of r estimats [2/3]
    - [X] Converted competition model parameters to r as in section
    - [X] Took the median of \(r\) estiamtes for each deletion
      - [X] Took median rather than mean to reduce the effect of
        outliers (might be caused by cross-contamination of cultures,
        pinning errors, or inocculation with dead
        cultures. (Biological variation could be another cause).)
    # - [ ] and calculated Spearman's rank correlation coefficient for
    #   both correlations.
    - [-] describe plot
      - [X] Plotted correlations between cultures (black) and between
        median values for each deletion (red) in figure.
      - [X] First I look at the correlation between estiamtes for
        each culture (black).
      - [X] There distribution is split into two distinct correlated
        groups.
	- [X] This is due to a gap in distribution of logistic model
          at around r = 4.
	- [X] In contrast there is no such gap in the distribution of
          competition model estimates.
	- [X] The groups only overlap for meduim values of competitoin
          model r
      - [X] For the main group there is a linear correlation with
        gradient close to one. However, logistic model estimates were
        hihger for almost all cultures in this group.
      - [X] Competiton model r varies more steeply with logistic model
        r for the outlying group.
      - [-] Extreme Outliers
	- [X] QFA R package has heuristic checks to correct for
          confounding between r and K for slow growing cultures. These
          are required becuase cultures are fit individually and slow
          growing cultures contain more noise.
	- [X] Outliers that were fit with r = 0 for the logistic model
          but higher values for the competiton model. Due to heuristic
          checks setting values to zero.
	- [X] Two outliers that were fit with very high r but very low K
          for the logistic model. This is a failure of the heruistic
          checks. Furthest is rad50 and other is est1
	- [ ] Competition model fits all cultures collectively using
          fewer parameters and does not require heuristic checks for
          slow growing cultures.
      - [X] Now describe distribution on medians
	- [X] Most median values fall inside the main group of cultures with lower logistic r
	- [X] However there are several that have both high
          competitoin and logistic model r that fall inside the
          outlying group.
	- [X] A significant number of r estimates fall between the two
          groups. This must mean that for many cultures repeats are
          split between the two black groups.
	- [X] This significantly worsens the correlation of r ranks
          between models.
	- [X] Spearman's rho measures the correlation between rank
          orders of two varaibles (i.e. a measure of how close to
          monotonical a relationship between two variables is). A
          perfectly monotonical distribution has a value of 1 or
          -1. Other correlations fall between these values.
	- [X] Between cultures Spearman's rho is 0.731; between
          medians for each deletion Spearman's rho is 0.497.
	- [X] Spearman's rho is imporved when comparing competiton
          model \(b\) (or equivalently \(r\)) with logistic model MDR see Figure rank.
  - [-] Comparison of ranking [5/6]
    - [X] b rank is equivalent to r and MDR rank for the competitino model.
    - [X] I compare ranking of deletions by competitoin model \(b\)
      rank with both types of fitness estimate for the logistic model.
    - [X] Used the median of parameter estimates from repeats of each deletion
    - [X] Compare logistic ranks
      - [X] est1 and rad50 are the outliers in correlation plot.
      - [X] MDR corrects and agrees with competition model
      - [X] Spearmans rho for competito model and MDR much better than
    - [X] How do fitness rankings between competition model and logistic model comparePu
      - [X] Agreement at the extremes
      - [X] Disagreement in the order of middle rankings
      - [X] 13 places between his3\Delta for b and MDR
    - [ ] Discussion: Validation data
  - [-] COV [3/4]
    - [X] I calculated the coeffifienct of variation between repeats
      of each deletion for competiton model esitmates of b and
      logistic model estimates of r.
    - [X] Chose to compare b and r because they both represent growth
      constants in the competiton and logistic models respectively
    - [X] In any case, COVs for MDR (not shown) are very similar to
      COVs for r.
    - [-] COVs are plotted for both models in Figure~\ref{fig:}
      - [X] Arranged from fittest to least fit from left to right by
        competiton model \(b\) ranking.
      - [X] Why study COV?
	- [X] biological variation of strains small
	- [X] Should get smilar estimates for strains regardless of
          location on the plate.
      - [X] Competitoin model had less COV for 36/50 strains
      - [X] Logistic model had least COV for top growers
	- [X] These are the upper right culture in correlation plot
          where there is no overlap in between the groups of cultures.
      - [ ] Neutral deletion his3 is shown in bold and COV in b is
        about half that in r.
      - [ ] For many deletions of medium fitness, logistic model \(r\)
        estimates of repeats are split across a distiance in logistic
        r between the two groups, separated. -> explaining why COV is
        higher in medium cultures.
      - [ ] The heuristic checks for the slowest growers creates the
        outliers which also have larger variation in r than b. -> Slow
        growers have higer variance.

* TODO Stripes Results [5/17]
 - [X] I fit the stripes and filled plates (link images) with the competition model.
 - [X] Data courtesty of Eva holstein. Can go in acknowledgements
 - [X] As for P15 but used different range of C(0)
 - [X] I took a Spline and take 15 timpoints
   (as described in fitting seciton).
 - [ ] Used a different range of C(0)
   - [ ] higher inoulum density
   - [ ] heterogeneity
 - [X] The experiment is designed to test for conpeitotn see methods for a description of the data
   - [X] The strains on the Filled plate are contained in the same positions on the Filled plate
   - [X] Can therfore estimate parameters on the filled plate, and
     simulate the Stripes plate by setting \(b\) for empty cultures
     zero for the empty cultures.
 - [ ] The competition model overcorrects for compeition; cell
   densities are higher than observed cells across the plate.
 - [ ] There was less agreement between the top five fits of both the
   Stipes and Filled plates than there was P15.
 - [ ] Top 5 Stripes Spearmans 0.988 - 0.998; top 3 b MAD 1.99-26.8
 - [ ] Filled Spearmans 0.97-0.998; b MAD 0.767-18.86
 - [ ] Not confident I was finding global minima
 - [ ] Despite having very different parameters for C(0), k, a
 - [ ] N(0) was well estimated
 - [ ] Callibration may not be working because I was not finding a
   global minimun.
 - [ ] Could use a genetic algorithm
   - [ ] With fixed plate level parameters and simulated noise can
     return \(b\)
...
* TODO Towards a genetic algorithm [/]
