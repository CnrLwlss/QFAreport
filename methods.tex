...
\subsubsection{\thesubsubsection~P15}
...
\citet{Addinall2011}. P15 is plate-15 from a standard deletion library
in \textit{S. Cerevisiae} with background mutation
\textit{cdc13-1}. \textit{cdc13-1} is a temperature dependent
conditional mutatation in a gene involved in telomere
stability. Internal cultures consist of 6 repeats of each of 49
different double mutants with deletions in genes thought to be
relevant to telomere function and 14 repeats of a strain with a
neutral deletion, \textit{his3}\(\Delta\). Edge cultures are also
inoculated with repeats of \textit{his3}\(\Delta\) but estimates from
these cultures are discarded due to noise in cell density estimates
caused by reflections from plate walls. Cultures were inoculated with
...
data exists for several strains in published studies using spot tests
\citep{maringele2002exo1,zubko2004exo1,Holstein20141259,foster2006mrx}.

\subsubsection{Stripes and Filled plates}
...
\begin{Figure}
  ...
  \captionof{figure}{\textbf{Schematic of the imaginary neighbour
    ...
    fast). The model uses the competition model rate equations
    (\ref{eq:competition_model}), with \(C_{i}\) and \(N_{i}\) for
    each culture, but with separate nutrient diffusion constants
    \(k_{s}\) and \(k_{f}\) for slow and fast growing neighbours
    rather than a single parameter \(k\). Each culture is modelled as
    having the same \(C(0)\) and \(N(0)\). \(k_{s}\), \(k_{f}\), and
    the growth constant \(b_{i}\) of the ``real'' are estimated by
    fitting the model to \(C_{obs}\) with all other parameters
    fixed. \(b_{i}\) is kept as an initial guess for fitting of the
    competition model. \(n\) of each neighbour are chosen to try to
    replicate the different configurations of neighbours that might be
    present on the real plate. \(C(0)\) and \(N(0)\) are fixed using
    previous guesses (see Section~\ref{sec:guessing_amounts}).}
