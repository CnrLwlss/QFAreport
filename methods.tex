...
\begin{Figure}
  ...
  \captionof{figure}{\textbf{Schematic of the imaginary neighbour
    ...
    \(C_{obs}\), is modelled growing alongside imagined slow and fast
    growing neighbours (blue) with growth constants \(b_{s}\) and
    \(b_{f}\) (for slow and fast). Rather than a single parameter, the
    model uses separate nutrient diffusion constants, \(k_{s}\) and
    \(k_{f}\), for slow and fast growing neighbours. \(k_{s}\),
    \(k_{f}\), and the growth constant, \(b_{i}\), of the real culture
