...
\subsection{\thesubsection~The Stripes and Filled plate datasets}
\label{sec:stripes_description}

The Stripes and Filled plates (Figure~\ref{fig:stripes_images}) are a
QFA experiment designed to study competition (personal communications
E. Holstein (April 2016)). On the Stripes plate
...
inoculated in the same positions on the Filled plate. These are
...
\textit{rad75}\(\Delta\) are generally fitter than the shared strains
with \textit{cdc13-1}. I higher inoculum denstiy (\(\sim\)10,000
...
the same strains of different plates; cultures on the Stripes plate
should have access to more nutrients because they grow next to empty
...
neighbours. I used the data from the Stipes and Filled plate for
cross-plate calibration and validation of the competition model by
simulating timecourses for the Stripes plate using parameters
estimated by fitting the Filled plate.
