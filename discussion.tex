...
and \ref{fig:P15_zone_fit}) use less parameters than fits of either
...
(see Table~\ref{tab:P15_obj_fun} and
...
Figure~\ref{fig:P15_zone_fit}.

In the correlation of logistic and competition model \(r\) estimates for
P15 (Figure~\ref{fig:P15_correlations}), there is a split in the
distribution of logistic model \(r\) estimates, but not competition
model \(r\) estimates. It is possible that the distribution of \(r\)
for strains (not for each strain) in the study of \citet{Addinall2011}
does have such a split distribution. However, repeats of many
individual strains fall in either group but not in the gap. This is
evidenced by the positions of medians in the gap (red). It seems
unlikely that the same split exists in the different distributions of
\(r\) for different strains and at the same value. It is possible that
the split is either an artefact of the heuristic checks conducted by
QFA R \citep{qfa2016} for slow growing strains, or caused by an
acceleration of faster growing strains and deceleration of slower
growing strains due to competition between neighbours. If the latter
case is true, the competition model appears to correct for it. It is
important to investigate the cause of the split distribution because
it will affect the significance given to genetic interactions.
...
Estimates of the nutrient diffusion constant \(k\) were fairly
