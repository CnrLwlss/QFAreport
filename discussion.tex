...
(see Figure~\ref{fig:comp_vs_log_ranking}. In independent validation
using spot cultures (ref) these are very sick strains and I confirmed
this in the raw QFA images by visual inspection. High \(r\) and low
\(K\) have been fit to both strains. This is corrected for when
converting to \(MDR\) which agrees with the competition model ranking
and independent validation. For other cultures it appears that
encroachment of fast growing cultures into neighbours is not being
picked up by Colonyzer. \textit{mre11\(\delta\)} is a weak growing
strain (ref validation) which was misclassified as healthy by the
competition model but not the logistic model. One repeat contained
unusual heterogeneity which may be natural or be the results of
contamination. (Should take median value next time). It may be that
the failure of these checks is leading to inaccurate estimates of
fitness for many strains. Due to a lack of independent validation it
is unclear which model to believe for the middle rankings.


\textit{zrt3\(\delta\)} does not appear to be as fast growing as the
logsitc model

\textit{hap4\(\delta\)}
%%

Some reactions are not fit as being complete at the end of
observations. Still in the exponential growth phase. Less nutrients in
the agar so that the stationary phase could be reached before cultures
started to merge.
...
% (Discussion: Recent work Herrmann and Lawless suggests
% that direct measures of C t 0 may not be reliable due to het-
% erogeneity between inoculated cells; many inoculated cells
% don’t grow and only the fastest growing cells contribute sig-
% nificantly to the final population.)
