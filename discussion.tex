...
Competition model fitness rankings of deletions on P15 are similar to
...
% Logistic model estimates for \textit{zrt3\(\Delta\)} might have been
% affected by noise and confounding between \(r\) and \(K\).
...
Fitting the logistic model to slower growing cultures required
...
% % \citep{zubko2004exo1,Addinall2011} which was misclassified as
% % healthy by the competition model when I was using mean values (not
% % shown) but not the logistic model. One repeat contained unusual
% % heterogeneity, which may be natural or the results of contamination,
% % and may explain the discrepancy. I corrected for this by taking
% % median values in Figure~\ref{fig:comp_vs_log_rankinkgsattempted to
% % correct for this issue by taking the median with partian nmedian
...
% As I lack independent data for validation it is
...
The distributions of logistic and competition \(r\) are very different
...
% Current best solutions, which are different
% local minima, systematically overcorrect for competition in a
% validation experiment (Figure~\ref{fig:stripes_validation}),
% suggesting that fundamental improvements to the competition model are
% also required.

The systematic overcorrection seen in the validation experiment
...
% The diffusion equation model of \citet{Reo2014} could be adapted for
% QFA to simulate the diffusion of nutrients in more detail. It would
% be computationally challenging to use such a detailed model to fit a
...
% A different type of diffusion limitation due to overcrowding inside
% cultures could cause a similar issue and might be resolved using a
% lower nutrient density.

If competition for nutrients is not responsible for the interaction
...
Recent work by Herrmann and Lawless suggests that directly measurement
...
\(b_{f}\). In reality cultures have different arrangements of
...
% Even with an improved guess, I suspect, based on the closeness of
% current fits (Figure~\ref{fig:P15_zone_fit}), that a gradient method
% will fail to find global minima.
\\\\
