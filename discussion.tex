...
because, due to the low inoculum density used, these cultures were
greatly affected by noise and the model is more flexible; it fits all
cultures independently with three culture level parameters, whereas
the competition model fits all cultures collectively with only one
culture level parameter. Estimated parameters might be less accurate
...
reflections off plate walls. Whereas the data is simply discarded for
the logsitic model, the competition model must fit these cultures. For
the competition model fit of P15, edge cultures contributed 70\% of
error in objective function value despite making up only 20\% of
cultures (Tables~\ref{tab:P15_obj_fun} and \ref{tab:corner}). This
will have affected all parameter estimates because cultures were fit
collectively with several plate level parameters. However, it will
have most greatly affected cultures neighbouring an edge. A two
\(N(0)\) parameter model improved the fit of these cultures
...
A model with an extra \(N(0)\) model  improved the The quality of fit was ibbbbb

Because of collective fitting, This will have affected
estimated plate level parameters and estimated \(b\) for cultures next
to a


a) These cultures dominate messes up other estimates.
b)


Edge cultures are more affected by noise from reflections off plate
walls and estimates are useually discarded. This can be seen in
Tables~\ref{tab:P15_obj_fun} and \ref{tab:corner} where, for the
competition model edge cultures contribute more than five times as much noise.

Noisy 5x edge cultures.



% There are large regions on P15 where there is little difference in
% growth between neighbouring cultures, e.g., the top left corner of
% Figure~\ref{fig:comp_fit_plate}, and competition will have had
% little effect on growth. In contrast, the 3x3 zone in
% Figure~\ref{fig:} has large differences in growth between
% neighbouring cultures. Plate comes from an actual experiment though
% so can't really complain about the layout.
